...
#
#   Generated Mon Nov 23 20:35:59 2020  by huyuxiang
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(RootIOUtil_tag)_install_more_includes.make
cmt_local_tagfile_install_more_includes = $(bin)$(RootIOUtil_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RootIOUtil_tag).make
cmt_local_tagfile_install_more_includes = $(bin)$(RootIOUtil_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RootIOUtil_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RootIOUtilsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RootIOUtil" = ""; then \
	    if test -d ../RootIOUtil; then \
	      echo "Installing files from more ../RootIOUtil to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RootIOUtil; then mkdir -p $(install_include_dir)/RootIOUtil; fi;\
	        $(install_command) ../RootIOUtil $(install_include_dir);\
	    else \
	      echo "No more RootIOUtil include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RootIOUtil"; \
	  $(uninstall_command) "$(install_include_dir)/RootIOUtil/*" ; \
	  if test -f $(install_include_dir)/RootIOUtil.cmtref ; then \
	    echo "Removing $(install_include_dir)/RootIOUtil.cmtref"; \
	    eval rm -f $(install_include_dir)/RootIOUtil.cmtref ; \
	  fi; \
	fi
