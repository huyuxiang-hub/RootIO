...
#
#   Generated Mon Nov 23 20:36:09 2020  by huyuxiang
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(RootIOSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RootIOSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RootIOSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RootIOSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RootIOSvc" = ""; then \
	    if test -d ../RootIOSvc; then \
	      echo "Installing files from more ../RootIOSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RootIOSvc; then mkdir -p $(install_include_dir)/RootIOSvc; fi;\
	        $(install_command) ../RootIOSvc $(install_include_dir);\
	    else \
	      echo "No more RootIOSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RootIOSvc"; \
	  $(uninstall_command) "$(install_include_dir)/RootIOSvc/*" ; \
	  if test -f $(install_include_dir)/RootIOSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/RootIOSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/RootIOSvc.cmtref ; \
	  fi; \
	fi
