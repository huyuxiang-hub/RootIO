...
#
#   Generated Mon Nov 23 20:56:12 2020  by huyuxiang
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(RootIOTest_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RootIOTest_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RootIOTest_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RootIOTestsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
	  ...
	  if test ! "Event" = ""; then \
	    if test -d ../Event; then \
	      ...
	      if test ! -d $(install_include_dir)/Event; then mkdir -p $(install_include_dir)/Event; fi;\
	        $(install_command) ../Event $(install_include_dir);\
	    else \
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  $(uninstall_command) "$(install_include_dir)/Event/*" ; \
	  if test -f $(install_include_dir)/Event.cmtref ; then \
	    echo "Removing $(install_include_dir)/Event.cmtref"; \
